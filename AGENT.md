...
### 0.2 把产物交给仓外工具（回轨）：同样禁止扫盘

AI 片段导出后要交给 `gtrk ai-drama lay` 回填，而那条命令吃的是**文件系统路径**。拿到项目 ID 后
按这两条**确定性**路径解析导出包位置，**MUST NOT 用 `find` / `ls -R` / 通配搜索去"先找到再说"**：

1. **服务可达** → `GET <API Base>/projects/<project-id>`，取响应里的 `exportDir`。
   **该字段缺席 = 这个项目还没导出**，此时报「尚未导出」并停下，不要据 ID 自己拼一个路径去试。
2. **服务没起** → 读 `~/.gitruck/ai-drama-desk.json` 的 `instances[]`，逐条试
   `<projectsDir>/<project-id>/exports/aidrama`。命中唯一即用；**一条都没命中时要报出已经查过哪些
   数据根**（好让「它在另一个部署里」一眼可见）；多条命中就报歧义并停下，不要自己挑一条。
3. 两条都不可用 → 说明如何启动工作台，或请用户直接给出导出包路径。**到此为止，不要扩大查找范围。**

> ⚠️ **数据根与安装位置不是一回事。** `DATA_DIR` 默认是 `<仓库>/data`，但可被
> `GITRUCK_DESK_DATA_DIR` 覆盖——打包版常见形态是**应用装在一个盘、数据在用户应用数据目录**。
> 用户报的「我装在 X」通常是安装位置，那里往往没有 `data/`。当刻真实数据根以
> `GET /health` 之外的两条路径为准：项目查询返回的 `dir`，或落脚点里的 `dataRoot`。
