...
| | 能力 | 做什么 |
|---|---|---|
| 📥 | 分镜稿导入与修改 | 粘贴分镜稿 md → 解析成分镜卡片；导入后可继续修改角色描述，以及每镜标题、时长、场景、出场角色、视觉基调、镜头描述与对应原文。保存后的文字用于下一次生成，既有候选不会被改写 |
...
>
> ⚠️ **产物根可以和数据根不在一处。** 界面「设置与诊断 → 数据目录 → 更改位置」可以把
> `projects/` 挪到别的盘（一条片子实测 341 MB，默认落在系统盘的用户目录）。改过之后
> **`dataRoot` 与 `projectsDir` 就不再是父子关系**——解析项目路径一律用 `projectsDir`，
> **MUST NOT 拿 `<dataRoot>/projects` 去拼**。`/api/paths` 的 `projectsRootIsDefault`
> 告诉你这台机器有没有改过。配置文件与密钥仍留在数据根，不跟着搬。
