...
2′. **不盲找产物**：把导出包交给 `gtrk ai-drama lay` 时，路径**只经两条确定性路径解析**——① 服务可达就取 `GET /projects/<id>` 的 `exportDir`（**字段缺席 = 尚未导出**，不要据 ID 拼路径去试）；② 服务没起就读 `~/.gitruck/ai-drama-desk.json` 的 `instances[]` 逐条试，零命中要报出已查过哪些数据根，多命中报歧义不猜。两条都不可用就请用户启动服务或直接给路径，**MUST NOT 扫盘**。⚠️ 数据根 ≠ 安装位置：打包版常见「应用在一个盘、数据在用户应用数据目录」，用户报的多半是后者以外的那个。
