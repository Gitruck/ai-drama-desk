...
export const api = {
  ...
  localEngine: () => req("/api/v1/local-engine"),
  setLocalModelsDir: (path: string) => req("/api/v1/local-engine/models-directory", { method: "PUT", body: JSON.stringify({ path }) }),
  installLocalEngine: () => req("/api/v1/local-engine/install", { method: "POST", body: JSON.stringify({ confirmed: true }) }),
  startLocalEngine: () => req("/api/v1/local-engine/start", { method: "POST" }),
